...
<script>
	...
	export default {
		...
		methods: {
			model:function(itemIndex) {
				this.$axios.get(this.url  + itemIndex * 10).then(function(response) {
					this.loading = true;
			...
			getMore:function(itemIndex) {
				this.switchShow = !this.switchShow;
				this.nowPage++;
				this.model(this.nowPage);
				console.log(this)
			},
			init:function(){
				this.busy = true;
	...
	/*export default {
		...
		methods: {
			moreFn: function(games) {
				this.$axios.post(this.url + itemIndex * 10).then(function(response) {
					this.busy = true;
					this.games = this.games.concat(response.body);
					this.switchShow = !this.switchShow;
					this.busy = false;
...
<style>
	...
	.loading {
		width: 75px;
		margin: 0 auto;
		background: url(../common/imgs/loading.gif) 0 center no-repeat;
		padding-left: 15px;
